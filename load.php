...
try {
    ...
    if ($num_rows > 0) {
        while ($row = $resultado->fetch_assoc()) {
            ...
            $tr_attributes .= 'data-anotacion="' . htmlspecialchars($row['anotacion'] ?? '', ENT_QUOTES, 'UTF-8') . '" ';
            ...
            $output['data'] .= '<td contenteditable="true" data-col="anotacion" data-id="' . htmlspecialchars($row['codigoprod'] ?? '', ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($row['anotacion'] ?? '', ENT_QUOTES, 'UTF-8') . '</td>';
